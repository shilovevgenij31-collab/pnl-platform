...
function buildPnlUserPrompt(data: FormInput): string {
  ...
  if (data.pnlText?.trim()) {
    lines.push('', '=== P&L / ФИНАНСОВЫЕ ДАННЫЕ ===')
    lines.push('Формат таблицы: первая колонка — название статьи, вторая колонка (Среднее) — среднее значение за все периоды, остальные колонки — отдельные периоды слева направо (от более раннего к более позднему). Используй колонку «Среднее» как источник средних значений; по остальным колонкам считай динамику и периоды.')
    lines.push('', data.pnlText)
  }
